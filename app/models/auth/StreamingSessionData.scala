...
@Singleton
class StreamingSessionDataImpl @Inject()(config: Config)(implicit val executionContext: ExecutionContext)
  extends StreamingSessionData with LazyLogging {

  private val secret: String = config.getString("play.http.secret.key")

  /**
    * createToken returns a Base64 encoded token, however it replaces the "+" and
    * "/" characters with "." and "-" to make the token safe for use in a URL.
    */
  def createToken(axonSessionToken: String, fileIds: SortedSet[UUID]): String = {
    val allFileIds: String  = fileIds.fold("") {(z, i) => z.toString + i.toString}.toString
    val token = Signature.calculateRFC2104HMAC(axonSessionToken + allFileIds, secret, Signature.HMAC_SHA256)
