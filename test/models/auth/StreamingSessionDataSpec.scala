...
class StreamingSessionDataSpec extends PlaySpec with MockitoSugar {
  ...
  "StreamingSessionData" must {
    ...
    "create token and verify against fixed value" in {
      ...
      val result = testSessionData.createToken(someToken, SortedSet[UUID](someUUID))
    ...
    "create token" in {
      ...
      val result = testSessionData.createToken(someToken, SortedSet[UUID](someUUID))
      val expected = Signature.calculateRFC2104HMAC(someToken + someUUID.toString, secret, Signature.HMAC_SHA256).replaceAll("\\+", ".").replaceAll("\\/", "_")
    ...
    "create token with some test axon token and empty file id set" in {
      ...
      val expected = Signature.calculateRFC2104HMAC(someToken + "", secret, Signature.HMAC_SHA256).replaceAll("\\+", ".").replaceAll("\\/", "_")
      result mustBe expected
    }

    "create token with empty axon token and empty file id set" in {
      val result = testSessionData.createToken("", SortedSet[UUID]())
      val expected = Signature.calculateRFC2104HMAC("", secret, Signature.HMAC_SHA256).replaceAll("\\+", ".").replaceAll("\\/", "_")
      result mustBe expected
    }
    ...
    "validate token" in {
      ...
      val streamingSessionToken: String =
        Signature.calculateRFC2104HMAC(someToken + someUUID.toString, secret, Signature.HMAC_SHA256).replaceAll("\\+", ".").replaceAll("\\/", "_")
      val result = testSessionData.validateToken(streamingSessionToken, someToken, SortedSet[UUID](someUUID))
      result mustBe true
    }
  }
