...
class MediaIdentSpec extends PlaySpec {
  ...
  "Media Identifier" must {
    "return a string for a single media identifier" in {
      ...
      val result: String = new MediaIdent(List(fileId), List(evidenceId), partnerId).toQueryString
    ...
    "return a string for a set of two media idents" in {
      ...
      val partnerId      = randomUUID
      val result: String = new MediaIdent(fileIds, evidenceIds, partnerId).toQueryString
      val expected: String =
        s"file_id=${fileIds(0)},${fileIds(1)}&evidence_id=${evidenceIds(0)},${evidenceIds(1)}&partner_id=$partnerId"
